...
def profile(request, pk):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            profile_img_file_exist = any(
                file.name == "profile_img.png" for file in request.FILES.getlist("img")
            )

            background_pic_form = UploadBackgroundPic(
                request.POST or None, request.FILES or None
            )
            background_img_file_exist = any(
                file.name == "background_img.png"
            ...
            if profile_pic_form.is_valid() and profile_img_file_exist:
                ...
                try:
                    ...
                    messages.error(
                        request, "Error occurred while uploading profile picture."
                    )
                ...
                try:
                    if BackgroundPicture.objects.filter(user_profile=profile).exists():
                        existing_background_picture = BackgroundPicture.objects.get(
                            user_profile=profile
                        )
...
def view_circle_messages(request):
    if request.user.is_authenticated:
        ...
        for circle in circles:
            circle_chat_room = circle.chat_room
            chat_rooms[circle_chat_room] = Message.objects.filter(room=circle_chat_room.id).order_by('-timestamp').first()

        chat_rooms = dict(sorted(chat_rooms.items(), key=lambda x: x[1].timestamp, reverse=True))
    ...
    if request.user.is_authenticated:
        ...
        for message_obj in messages:
            serialized_message = {
                ...
                "sender_pic_url": message_obj.sender.profile.profile_picture.img.url,
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if Profile.objects.filter(
                friend=receiver_user.profile, user=request.user
            ).exists():
                messages.info(
                    request, "You are already friends with " + receiver_user.username
                )
                return redirect("friends")
            ...
            if FriendRequest.objects.filter(
                ...
                messages.info(
                    request,
                    "You already have a friend request from " + receiver_user.username,
                )
                return redirect("friends")
            ...
            if FriendRequest.objects.filter(
                ...
                messages.info(
                    request,
                    "You already sent a friend request to " + receiver_user.username,
                )
                return redirect("friends")
